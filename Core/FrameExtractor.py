...
def run(videos_path:str, save_path:str, frame_size:int, workers:int, original_size:bool):
    # path checking
    ...
    if path_manager(save_path, raise_error=False, path_exist=True):
        print(f"Path {save_path} already exists, skip this step...")
        return True
    else:
    ...
    Parallel(n_jobs=workers, backend="threading")(delayed(frame_extractor)(
        [i, len(videos_path)], video_path, start_point_of_path, save_path, frame_size, original_size
    ) for i, video_path in enumerate(videos_path))

def frame_extractor(index:int, video_path:str, start_point_of_path:int, frame_path:str, frame_size:int, original_size:bool):
    ...
    (
        ...
        .output(os.path.join(frame_path, "%d.jpeg"))
